...
-- ================================================================================
-- Generated from all Flyway migrations (V1 through V17).
...
-- ================================================================================
-- TABLE: public.users
-- Stores admin users who can access the web administration interface
-- ================================================================================
...
-- Applications that expose their own status pages
-- ================================================================================
...
COMMENT ON COLUMN public.status_apps.name IS 'Display name of the status application';
...
-- Logical components or subsystems of a status application
-- ================================================================================
...
-- Incidents representing service disruptions or outages for an application
-- ================================================================================
...
-- Composite index for public status page lookups (V16)
...
-- Timeline of status updates and messages associated with an incident
-- ================================================================================
...
COMMENT ON COLUMN public.status_incident_updates.status IS 'Incident status at the time of this update';
...
-- Mapping between incidents and the components they affect
-- ================================================================================
...
-- Scheduled maintenance windows for an application
-- ================================================================================
...
-- Mapping between scheduled maintenance entries and the components they affect
-- ================================================================================
...
-- One record per day per app (component_id NULL)
...
-- Events logged from platforms and components for monitoring and debugging
-- ================================================================================
...
-- GIN trigram index for full-text search on message (V16)
CREATE INDEX idx_logs_message_trgm ON public.logs USING gin (lower(message) gin_trgm_ops);
...
CREATE INDEX idx_log_metrics_service_level_bucket
...
-- API keys used to authenticate log ingestion requests (final state after V15/V17)
-- Note: plaintext api_key column was replaced by key_hash + key_prefix
-- ================================================================================
...
CREATE TABLE public.log_api_keys (
    ...
    tenant_id               UUID
        ...
        ON DELETE CASCADE,
    name                    VARCHAR(255) NOT NULL,
    key_hash                VARCHAR(64)  NOT NULL,   -- SHA-256 hex digest of the API key
    key_prefix              VARCHAR(8)   NOT NULL,   -- First 8 characters for display only
    is_active               BOOLEAN      DEFAULT true,
    created_date            TIMESTAMP WITH TIME ZONE NOT NULL,
    created_date_technical  BIGINT       NOT NULL,
...
COMMENT ON TABLE  public.process_mining_retention_rules IS 'Per-platform log retention policies controlling how long logs are stored';
...
COMMENT ON COLUMN public.process_mining_retention_rules.enabled IS 'Whether the retention policy is active';
COMMENT ON COLUMN public.process_mining_retention_rules.last_run_at IS 'Timestamp of the last retention cleanup run';
COMMENT ON COLUMN public.process_mining_retention_rules.last_run_deleted_count IS 'Number of log records deleted in the last run';

-- ================================================================================
-- V18: BRIN indexes for sequential/timeseries columns
-- BRIN (Block Range INdex) summarises min/max per page range. Ideal when physical
-- insertion order correlates with column values. ~100-1000x smaller than B-tree,
-- near-zero INSERT overhead, efficient range scans for time-window queries.
-- pages_per_range: 32 = high-volume; 64 = medium; 128 = low-volume (default).
-- ================================================================================

-- logs.log_timestamp: always advances as logs are ingested (append-only)
...
-- log_metrics.bucket: time-aligned aggregation window, always moves forward
...
-- platform_events.event_time: events recorded at occurrence time, sequential with ingestion
...
-- platform_events.created_date_technical: epoch-millis insertion timestamp, strictly monotonic
...
-- status_uptime_history.record_date: one row per app/component per calendar day, appended forward
...
-- status_incidents.started_at: incidents created when they occur, insertion order matches start time
...
-- status_maintenance.starts_at: records created near scheduled time, insertion order tracks start time
...
-- status_incident_updates.update_time: updates written in chronological order as incidents progress
