...
class StatusMaintenanceRepositoryTest {
    ...
    void findByStatus_filtersByStatus() {
        persistMaintenance("Scheduled", "SCHEDULED", now, now.plusHours(1), app);
    ...
    void findByAppIdAndStatus_combinesAppAndStatus() {
        ...
        assertThat(repository.findByAppIdAndStatus(app.getId(), "SCHEDULED"))
                .extracting(StatusMaintenance::getTitle).containsExactly("Scheduled");
    }
    ...
    void findByStartsAtBetween_returnsWindowsStartingInRange() {
        persistMaintenance("InRange", "SCHEDULED", now.plusHours(1), now.plusHours(2), app);
        ...
        assertThat(repository.findByStartsAtBetween(now, now.plusHours(3)))
                .extracting(StatusMaintenance::getTitle).containsExactly("InRange");
    }

    @Test
    void findByEndsAtBetween_returnsWindowsEndingInRange() {
        persistMaintenance("InRange", "SCHEDULED", now, now.plusHours(2), app);
    ...
    void search_matchesTitleOrDescription() {
        StatusMaintenance m = persistMaintenance("DB upgrade", "SCHEDULED", now, now.plusHours(1), app);
        ...
        assertThat(repository.search("upgrade"))
                .extracting(StatusMaintenance::getTitle).containsExactly("DB upgrade");
